...
class Timer(SafeStruct):
  ...
  def update(self, cycles, __mod__=operator.__mod__):
    ...
    while self.rticnt >= self.rtilimit: self.rticnt -= self.rtilimit
  ...
  def readTIC123(self, addr, bits, val, rw):
    write = self.memory.writeRawUns16
    if addr == self.memory.TIC1: write(self.memory.TIC1, self.tic1)
    elif addr == self.memory.TIC2: write(self.memory.TIC2, self.tic2)
    elif addr == self.memory.TIC3: write(self.memory.TIC3, self.tic3)
...
def install(sim, parentWindow):
  ...
  addfilter(filter(T.memory.TCNT, T.memory.TCNT+1, None, T.readTimer, None))

  # Register memory handler to handle TFLG1 register
  addfilter(filter(T.memory.TFLG1, T.memory.TFLG1, None, T.readTflg1, T.writeTflg1))

  # Register memory handler to handle TFLG2 register
  addfilter(filter(T.memory.TFLG2, T.memory.TFLG2, None, T.readTflg2, T.writeTflg2))

  # Register memory handler to handle writes to CFORC
  addfilter(filter(T.memory.CFORC, T.memory.CFORC, None, None, T.writeCFORC))

  # Register memory handler to handle reads of TIC1-TIC4
  addfilter(filter(T.memory.TIC1, T.memory.TIC3+1, None, T.readTIC123, None))
  addfilter(filter(T.memory.TIC4, T.memory.TIC4, None, T.readTIC4, None))

  # Register memory handlers for writes of TMSK1, TMSK2, PACTL, TOC1-TOC5 (for caching)
  addfilter(filter(T.memory.TMSK1, T.memory.TMSK1, None, None, T.writeTMSK1))
  addfilter(filter(T.memory.TMSK2, T.memory.TMSK2, None, None, T.writeTMSK2))
  addfilter(filter(T.memory.PACTL, T.memory.PACTL, None, None, T.writePACTL))
  addfilter(filter(T.memory.TOC1, T.memory.TOC5+1, None, None, T.writeTOC12345))

  # If input capture events are generated by the PIO module, save the times
  addhandler(T.events.IC1, T.handleIC1F)
  addhandler(T.events.IC2, T.handleIC2F)
  addhandler(T.events.IC3, T.handleIC3F)
  addhandler(T.events.IC4, T.handleIC4F)

  # Pulse accumulator events are generated by the PIO module but we handle
  # the flags.
  addhandler(T.events.PAI, T.handlePAI)
  addhandler(T.events.PAOV, T.handlePAOV)
